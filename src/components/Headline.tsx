import React from 'react';

const Headline = () => {
    return (
        <>
            <div className='row-start-2 col-span-1'>
                <h1 className='2xl:text-6xl text-[40px] text-Very-dark-blue font-extrabold leading-none'>The Bright Future of Web 3.0?</h1>
            </div>
            <div  className='mb-8 2xl:mt-0 mt-4 row-start-2 col-span-1'>
                <div className='h-full flex flex-col justify-between gap-6'>
                    <p className='text-[15px] text-Dark-grayish-blue leading-[26px]'>
                        We dive into the next evolution of the web that claims to put the power of the platforms back into the hands of the people. 
                        But is it really fulfilling its promise?
                    </p>
                    <button className='w-fit text-Off-white text-sm font-[500] tracking-[0.25rem] bg-Soft-red hover:bg-Very-dark-blue py-4 px-8'>READ MORE</button>
                </div>
            </div>
        </>
    );
};

export default Headline;

// 

// 

// Read more